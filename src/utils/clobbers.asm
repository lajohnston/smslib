...
; supports jumps outside the clobber scope using the utils.clobbers.end.x
; macros.
;
; As these jumps don't know whether the jump location is inside or outside the
; preserve scope, withBranching wraps the clobber scope with its own preserve
; scope to isolate it. The jumps can therefore determine to always restore the
; registers they clobber.
;
; @in   ...registers    list of registers that will be clobbered
;                       Valid values:
;                           "AF", "BC", "DE", "HL", "IX", "IY", "I",
